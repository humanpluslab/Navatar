...
public class ParticleFilter {
  ...
  public ParticleFilter(String filePath, String username, BuildingMapWrapper map,
      ParticleState startingState) {
    rand = new Random();
    ...
    File dir = new File(filePath + "/Profiling");
    if (!dir.exists())
      dir.mkdir();
    dir = new File(filePath + "/Particles");
    if (!dir.exists())
      dir.mkdir();
    logs = new TextFile(filePath + "/Logs/" + username + ".txt");
    try {
      logs.writeFile(false);
    } catch (IOException e) {
      e.printStackTrace();
    }
    ...
    if (printData) {
      xml = new XmlFile(filePath + "/Particles/" + username + "_run_" + run + "_Particles_"
          + NUM_OF_PARTICLES + ".xml");
      xml.append("<particles num=\"" + NUM_OF_PARTICLES_PER_PF + "\" num_pfs=\""
          + NUM_OF_PARTICLE_FILTERS + "\" ground_truth=\"" + this.groundTruthFile + "\">\n");
      try {
        xml.writeFile(false);
      } catch (IOException e) {
      ...
      xml = new XmlFile(filePath + "/Particles/" + username + "_run_" + run + "_Particles_"
          + NUM_OF_PARTICLES + ".xml");
      xml.append("<particles num_pfs=\"" + NUM_OF_PARTICLE_FILTERS + "\" ground_truth=\""
          + this.groundTruthFile + "\">\n");
      xml.append(" <step time=\"" + System.currentTimeMillis() + "\" meanX=\""
          + startingState.getX() + "\" meanY=\"" + startingState.getY() + "\" >\n");

      for (int i = 0; i < NUM_OF_PARTICLE_FILTERS; ++i)
        xml.append("  <pf step=\"" + twoDForm.format(stepM[i]) + "\" meanX=\""
            + twoDForm.format(pfEllipses[i][0]) + "\" meanY=\"" + twoDForm.format(pfEllipses[i][1])
            + "\" covX=\"" + twoDForm.format(pfEllipses[i][2]) + "\" covXY=\""
            + twoDForm.format(pfEllipses[i][3]) + "\" covY=\"" + twoDForm.format(pfEllipses[i][4])
            + "\" />\n");
